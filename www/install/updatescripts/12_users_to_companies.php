<?php
if(\GO\Base\Db\Utils::fieldExists('core_user', 'company') && \GO::modules()->addressbook){
	$ab = \GO\Addressbook\Model\Addressbook::model()->findSingleByAttribute('users', '1');//\GO::t("Users"));

	$pdo = \GO::getDbConnection();

	$pdo->query("INSERT INTO ab_companies (`addressbook_id`,`name`, `email`, `country`, `state`, `city`, `zip`, `address`, `address_no`) SELECT {$ab->id},`company`, `email`, `country`, `state`, `city`, `zip`, `address`, `address_no`  FROM `core_user` WHERE company!=''");
