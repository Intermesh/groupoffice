...
update pa_aliases set
                      address = replace(address, concat('@', @old_domain), concat('@', @new_domain)),
                      goto = replace(goto, concat('@', @old_domain), concat('@', @new_domain));

insert ignore into pa_aliases
select null,domain_id,replace(address, concat('@', @new_domain), concat('@', @old_domain)),goto,ctime,mtime,active from pa_aliases;
