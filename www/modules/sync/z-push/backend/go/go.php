...
require_once("backend/go/config.php");
require_once("backend/go/goSyncUtils.php");
require_once("backend/go/goImporter.php");
require_once("backend/go/goExporter.php");
...
//			throw new StatusException();
	...
	/**
	 * Get the contacts that are found in the user's default addressbook and 
	 * return it to the phone
	 * 
	 * @param StringHelper $searchquery
	 * @param StringHelper $searchrange
	 * @return array
	 * @throws Exception Throwed when no default addressbook is found for the GO user.
	 */
	public function GetGALSearchResults($searchquery, $searchrange) {
//		ZLog::Write(LOGLEVEL_INFO,'ZPUSH2Search->GetGALSearchResults::$searchquery'.$searchquery.' & $searchrange '.$searchrange.'');
//		
...
//		}
//		
//		// Retreive the default addressbook for the current user
////		$addressbook = $this->_getSettings()->getDefaultAddressbook();
////		if (!$addressbook)
////			throw new \Exception("FATAL: No default addressbook configured");
//		
//		// Set the field where to search in
//		$searchFields = \GO\Addressbook\Model\Contact::model()->getFindSearchQueryParamFields();
//		$searchFields[]="c.name";
//
//		// Build the findparams for this search
//		$findParams = \GO\Base\Db\FindParams::newInstance();
//		$findParams->searchQuery('%'.preg_replace ('/[\s*]+/','%', $searchquery).'%');
//		$findParams->searchFields($searchFields);
//	//	$findParams->getCriteria()->addCondition('addressbook_id',$addressbook->id);
//		$findParams->joinCustomFields();
//		$findParams->calcFoundRows();
//		$findParams->limit($rangeend);
//		$findParams->start($rangestart);
//		$findParams->order('last_name');
...
//		
//		// Loop through the results and fill the returning items array.
//		foreach($contacts as $contact){
//			$items[] = array(
//										SYNC_GAL_DISPLAYNAME    => $contact->name,
//                    SYNC_GAL_PHONE          => $contact->work_phone,
//                    SYNC_GAL_OFFICE         => 'OFFICE',
//                    SYNC_GAL_TITLE          => $contact->title,
//                    SYNC_GAL_COMPANY        => $contact->company?$contact->company->name:'',
//                    SYNC_GAL_ALIAS          => 'ALIAS',
//                    SYNC_GAL_FIRSTNAME      => $contact->first_name,
//                    SYNC_GAL_LASTNAME       => $contact->last_name,
//                    SYNC_GAL_HOMEPHONE      => $contact->home_phone,
//                    SYNC_GAL_MOBILEPHONE    => $contact->cellular,
//                    SYNC_GAL_EMAILADDRESS   => $contact->email
//									);
//		}
//ZLog::Write(LOGLEVEL_INFO,'ZPUSH2Search::GetGALSearchResults::'.$items['searchtotal'].' '.$items['range']);
////		ZLog::Write(LOGLEVEL_INFO,'ZPUSH2Search::GetGALSearchResults::results - '.var_export($items,true));
//		
//
//
//		return $items;
