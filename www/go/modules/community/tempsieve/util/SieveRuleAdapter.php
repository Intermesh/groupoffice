...
final class SieveRuleAdapter
{
	public array $content = [];      // script rules array

	public ?Sieve $sieve = null;

	private array $vars = [];        // "global" variables
	private string $prefix = '';           // script header (comments)
	private array $supported = [     // Sieve extensions supported by class
		'body',                     // RFC5173
		'copy',                     // RFC3894
		'date',                     // RFC5260
		'enotify',                  // RFC5435
	 ...
	 */
	public function __construct(string $script, Sieve $sieve, array $capabilities = array(), bool $getCapabilitiesFromServer = false)
	{
		...
		if ($getCapabilitiesFromServer) {
			...
			$capabilities = $this->sieve->getSieveExtensions();
		}
	 ...
	 *
	 * @param string $content Rule name
	 * @param int $index
	 *
	 * @return int The index of the new rule
	 */
	public function addRule(string $content, int $index = -1)
	{
		if ($index > -1) {
	...
	public function deleteRule(int $index): bool
	{
	...
	public function updateRule(int $index, string $content): bool
	{
	 ...
	 */
	public function setVar(string $name, string $value, array $mods = []): void
	{
	 ...
	 */
	public function setPrefix(string $text): void
	{
		$this->prefix = $text;
	}
		...
		if (!empty($this->vars)) {
			...
			foreach ($this->vars as $var) {
				if (empty($has_vars)) {
					...
					$output .= self::escapeString($var['name']) . ' ' . self::escapeString($var['value']) . ";\n";
				}
			}
		}
			...
			if (!empty($rule['tests'])) {
				foreach ($rule['tests'] as $test) {
					...
					switch ($test['test']) {
						...
						case 'exists':
							$tests[$i] .= ($test['not'] ? 'not ' : '');
							$tests[$i] .= 'exists ' . self::escapeString($test['arg']);
							break;
						...
						case 'header':
							...
							$this->addIndex($test, $tests[$i], $exts);
							$this->addOperateor($test, $tests[$i], $exts);

							$tests[$i] .= ' ' . self::escapeString($test['arg1']);
							$tests[$i] .= ' ' . self::escapeString($test['arg2']);
							break;
						...
						case 'body':
							...
							if (!empty($test['part'])) {
								...
								if (!empty($test['content']) && $test['part'] == 'content') {
									$tests[$i] .= ' ' . self::escapeString($test['content']);
								}
							}

							$this->addOperateor($test, $tests[$i], $exts);

							$tests[$i] .= ' ' . self::escapeString($test['arg']);
							break;
						...
						case 'currentdate':
							...
							if (!empty($test['originalzone']) && $test['test'] == 'date') {
								...
								$tests[$i] .= ' :zone ' . self::escapeString($test['zone']);
							}

							$this->addOperateor($test, $tests[$i], $exts);

							if ($test['test'] == 'date') {
								$tests[$i] .= ' ' . self::escapeString($test['header']);
							}

							$tests[$i] .= ' ' . self::escapeString($test['part']);
							$tests[$i] .= ' ' . self::escapeString($test['arg']);
				...
				foreach ($rule['actions'] as $action) {
					...
					switch ($action['type']) {
						...
						case 'fileinto':
							...
							$action_script .= self::escapeString($action['target']);
							break;
						...
						case 'ereject':
							...
							$action_script .= $action['type'] . ' '
								. self::escapeString($action['target']);
							break;
						...
						case 'removeflag':
							...
							$action_script .= $action['type'] . ' ' . self::escapeString($action['target']);
							break;
						...
						case 'set':
							...
							$action_script .= self::escapeString($action['name']) . ' ' . self::escapeString($action['value']);
							break;
						...
						case 'notify':
							...
							foreach (array('id', 'importance', 'method', 'options', 'from', 'message') as $n_tag) {
								if (!empty($action[$n_tag])) {
									$action_script .= " :$n_tag " . self::escapeString($action[$n_tag]);
								}
							}

							if (!empty($method)) {
								$action_script .= ' ' . self::escapeString($method);
							}
						...
						case 'vacation':
							...
							if (!empty($action['addresses']))
								$action_script .= " :addresses " . self::escapeString($action['addresses']);
							if (!empty($action['subject']))
								$action_script .= " :subject " . self::escapeString($action['subject']);
							if (!empty($action['handle']))
								$action_script .= " :handle " . self::escapeString($action['handle']);
							if (!empty($action['from']))
								$action_script .= " :from " . self::escapeString($action['from']);
							if (!empty($action['mime']))
								$action_script .= " :mime";
							$action_script .= " " . self::escapeString($action['reason']);
							break;
					}
	 ...
	 */
	private function parse(string $script): void
	{
		$prefix = '';
		$options = array();
		...
		while ($script) {
			...
			while (!empty($script) && $script[0] == '#') {
				...
				if (preg_match('/^# rule:\[(.*)\]/', $line, $matches)) {
					...
					$this->setVar($matches[1], $matches[2]);
				} // Horde-Ingo format
	 ...
	 */
	private function _tokenize_rule(string &$content): ?array
	{
	 ...
	 */
	private function parseActions(string &$content, string $end = '}')
	{
		$result = null;
		...
		while (strlen($content)) {
			...
			switch ($token) {
				...
				case 'redirect':
					...
					$args = array('copy');
					$action += $this->actionArguments($tokens, $args);
				...
				case 'vacation':
					...
					$action += $this->actionArguments($tokens, $args, $vargs);
	 ...
	 */
	private function addComparator(array $test, string &$out, array &$exts)
	{
		...
		if ($test['comparator'] != 'i;ascii-casemap') {
			$out .= ' :comparator ' . self::escapeString($test['comparator']);
		}
	}
	 ...
	 */
	private function addIndex(array $test, string &$out, array &$exts)
	{
		if (!empty($test['index'])) {
	 ...
	 */
	private function addOperateor(array $test, string &$out, array &$exts)
	{
		if (empty($test['type'])) {
			return;
		}
		...
		$this->addComparator($test, $out, $exts);
	}

	/**
	 * Extract test tokens
	 */
	private function testTokens(&$tokens): array
	{
		$test = array();
		$result = array();
	 ...
	 */
	private function actionArguments(&$tokens, $bool_args, $val_args = array()): array
	{
		$action = array();
		$result = array();
	 ...
	 */
	static function escapeString(string $str): string
	{
		if (is_array($str) && count($str) > 1) {
			foreach ($str as $idx => $val)
				$str[$idx] = self::escapeString($val);
		...
		if (preg_match('/[\r\n\0]/', $str) || strlen($str) > 1024) {
			return sprintf("text:\n%s\n.\n", self::escapeMultilineString($str));
		} // quoted-string
		else {
	 ...
	 * @return string Text
	 */
	static function escapeMultilineString(string $str): string
	{
	 ...
	 */
	static function tokenize(string &$str, $num = 0)
	{
		$result = array();
