...
export class PreferencesWindow extends FormWindow {
	constructor() {
		...
		this.generalTab.items.add(containerfield({name:'calendarPreferences'},
			...
			checkbox({name:'autoAddInvitations',label:t('Automaticaly add invitation to your calendar'),
				hint: t('Wheneven an event invitation is received, add the event to your default calendar')}),
			checkbox({name:'autoUpdateInvitations', label: t('Automaticaly apply updates from organizer'),
			...
			select({name:'weekViewGridSnap', label: t('Snap size for week / day view'),
				hint: t('The duration adjustment when resizing the event'),options: [
					{value:'5', name: t('5 minutes')},
					{value:'15',name:  t('15 minutes')},
					{value:'30',name:  t('30 minutes')},
					{value:'60',name:  t('1 hour')}
				]}),
			...
			select({name:'defaultDuration', label:t('Default duration'),
				...
				{value:'PT30M', name: t('30 minutes')},
				{value:'PT1H',name:  t('1 hour')},
				{value:'PT2H',name:  t('2 hours')},
				{value:null,name:  t('Full day')}
			]})
		));
	}
}