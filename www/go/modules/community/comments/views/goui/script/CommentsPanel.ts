		...
		this.items.add(comp({
			...
			tbar({},
				...
				btn({
					icon: "expand_less",
					handler: (button) => {
						this.commentList.hidden = !this.collapsed;
						this.commentEditor.hidden = !this.collapsed;
						this.bottomBar.hidden = !this.collapsed;
						this.countBadge.hidden = this.collapsed;

						button.icon = this.collapsed ? "expand_less" : "expand_more";
