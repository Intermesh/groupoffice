...
export function tasklistchips(config?: Partial<AutoCompleteChipsConfig<Table<DataSourceStore>> & {filter: Record<string, any>}>) {
	return autocompletechips({
		...
		chipRenderer: async (chip, value) => {
			const record = await jmapds("TaskList").single(value);
