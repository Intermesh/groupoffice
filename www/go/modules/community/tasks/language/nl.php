<?php
return array (
  'name' => 'Taken',
  'description' => 'Maak en beheer taken, plaats ze in lijsten, plan je dag en deel ze met andere gebruikers.',
  'mayChangeTasklists' => 'Takenlijsten bewerken',
  'mayChangeCategories' => 'Categorieën bewerken',
  'Select list' => 'Selecteer takenlijst',
  'Select category' => 'Selecteer categorie',
  'Task' => 'Taak',
  'Tasks' => 'Taken',
  'start' => 'begin',
  'due' => 'eind',
  'completed' => 'voltooid',
  'title' => 'titel',
  'status' => 'status',
  'priority' => 'prioriteit',
  'percentage completed' => 'percentage voltooid',
  'remind date' => 'herinnering (datum)',
  'remind time' => 'herinnering (tijd)',
  'categories' => 'categorieën',
  'Categories' => 'Categorieën',
  'Priority' => 'Prioriteit',
  'Open file' => 'Open bestand',
  'Tasklist not selected' => 'Takenlijst niet geselecteerd',
  'You have not selected any tasklist. Select a tasklist before proceeding.' => 'Er is geen takenlijst geselecteerd. Selecteer een takenlijst om verder te gaan.',
  'Choose a tasklist' => 'Kies een takenlijst',
  'In progress' => 'In behandeling',
  'Completed' => 'Voltooid',
  'Unplanned' => 'Niet gepland',
  'Failed' => 'Mislukt',
  'Needs action' => 'Actie nodig',
  'Cancelled' => 'Geannuleerd',
  'Due date' => 'Eind datum',
  'Add a task...' => 'Een taak toevoegen...',
  'Future tasks' => 'Toekomstige taken',
  '% complete' => '% compleet',
  'Due at' => 'Verloopt op',
  'Start at' => 'Start op',
  'Start time' => 'Begintijd',
  'Responsible' => 'Verantwoordelijke',
  'Start' => 'Begin',
  'Due' => 'Verloopt',
  'Percent complete' => 'Percentage compleet',
  'Estimated duration' => 'Geschatte duur',
  'Hours booked' => 'Geboekte uren',
  'Add alert' => 'Alert toevoegen',
  'Progress' => 'Voortgang',
  'Continue task' => 'Taak voortzetten',
  'Nothing to save' => 'Niets om op te slaan',
  'No changes have been made' => 'Er zijn geen veranderingen aangebracht.',
  'Duration' => 'Duur',
  'This task already has booked hours or progress' => 'Op deze taak zijn reeds uren of voortgang geboekt',
  'When' => 'Wanneer',
  'Global category' => 'Globale categorie',
  'Unscheduled' => 'Ongepland',
  'Scheduled' => 'Gepland',
  'Show completed' => 'Toon voltooide',
  'Completed tasks' => 'Voltooide taken',
  'You were assigned to this task by {assignedBy}' => 'Deze taak is aan u toegewezen door {assignedBy}',
  'Visible lists' => 'Zichtbare lijsten',
  'Display options for lists' => 'Weergave instellingen voor lijsten',
  'Default list' => 'Standaard takenlijst',
  'Remember last selected list' => 'Onthoud mijn laatst geselecteerde lijsten',
  'Set today for start and due date when creating new tasks' => 'Gebruik vandaag als standaardwaarde voor start- en verloopdatum voor nieuwe taken',
  'Assign me' => 'Toewijzen aan mijzelf',
  'Are you sure you want to unlink this item?' => 'Weet u zeker dat u dit item wilt ontkoppelen?',
  'Group name' => 'Groepsnaam',
  'Assigned' => 'Toegewezen',
  'Mine' => 'Mijn',
  'Unassigned' => 'Niet toegewezen',
  'Lists' => 'Lijsten',
  'Private notes' => 'Privé notities',
  'Contracts' => 'Contracten',
  'This customer doesn\'t have an active contract' => 'Dese klant heeft geen actief contract',
  'Assignment' => 'Toegewezen',
  'Alerts' => 'Meldingen',
  'Import list ID from CSV file' => 'Importeer lijst-id uit CSV-bestand',
  'Role' => 'Rol',
  'List' => 'Lijst',
  'Ok' => 'OK',
  'Open' => 'Open',
  'Unknown user' => 'Onbekende gebruiker',
  'You were assigned to this task by {assigner}' => 'Je bent toegewezen aan deze taak door {assigner}',
  'A new task was created in your list by {creator}' => 'Een nieuwe taak werd in uw lijst aangemaakt door {creator}',
  'Ticket' => 'Ticket',
  'CC' => 'CC',
  'Start in' => 'Beginnen',
  'Default tasklist' => 'Standaard takenlijst',
  'Remember last selected tasklist' => 'Laatste geselecteerde takenlijst onthouden',
  'List not selected' => 'Lijst niet geselecteerd',
  'You have not selected any list. Select a list before proceeding.' => 'U heeft geen lijst geselecteerd. Selecteer een lijst voordat u verder gaat.',
  'week' => 'week',
  'Tasklist' => 'Takenlijst',
  'Status' => 'Status',
  'Show project tasks' => 'Projecttaken tonen',
  'Web page' => 'Webpagina',
  'Subscribe to Tasklist' => 'Abonneren op takenlijst',
  'Project' => 'Project',
  'Sync to device' => 'Synchroniseren naar apparaat',
  'Make tasklist available in CalDAV and ActiveSync' => 'Taaklijst beschikbaar maken in CalDAV en ActiveSync',
  'Create tasklist' => 'Takenlijst aanmaken',
  'Subscribe to task list' => 'Abonneren op takenlijst',
);
