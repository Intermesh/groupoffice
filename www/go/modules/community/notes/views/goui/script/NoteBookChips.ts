...
export function notebookchips(config?: Partial<AutoCompleteChipsConfig<Table<DataSourceStore>> & {filter: Record<string, any>}>) {
	return autocompletechips({
		...
		chipRenderer: async (chip, value) => {
			const record = await jmapds("NoteBook").single(value);
