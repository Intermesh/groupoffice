...
class Sync extends CronJob {
    ...
    ->select('id,syncGroups,syncUsers,syncUsersDelete,syncGroupsDelete')
    ...
    foreach($records as $record) {
      if($record['syncUsers']) {
        $c->users($record['id'], false, !empty($record['syncUsersDelete']));
      }

      if($record['syncGroups']) {
        $c->groups($record['id'],false, !empty($record['syncGroupsDelete']));
      }
    }
	}
}
