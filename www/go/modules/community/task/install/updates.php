...
$updates['201911061630'][] = function(){
	...
	while($row = $stmt->fetch()){
		...
		GO()->getDbConnection()->insert('task_portlet_tasklist', $data)->execute();
	}
	...
	while($row = $stmt->fetch()){
		$data = [];
		$data['id'] = $row["id"];
		GO()->getDbConnection()->insert('task_tasks_custom_field', $data)->execute();
	}
	...
	while($row = $stmt->fetch()){
		...
		$data['name'] = $row["name"];
		$data['createdBy'] = $row["user_id"];
		GO()->getDbConnection()->insert('task_category', $data)->execute();
	}
	...
	while($row = $stmt->fetch()) {
		...
		GO()->getDbConnection()->insert('task_task', $data)->execute();
	}
	...
	while($row = $stmt->fetch()) {
		...
		GO()->getDbConnection()->insert('task_alert', $data)->execute();
	}
};
