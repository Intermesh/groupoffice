...
class Language {
	...
	public function getIsoCode(): string
	{
		if(isset($_GET['SET_LANGUAGE']) && $this->hasLanguage($_GET['SET_LANGUAGE'])) {
			if(!headers_sent()) {
				setcookie("GO_LANGUAGE", $_GET['SET_LANGUAGE'],time() + (10 * 365 * 24 * 60 * 60),"/; samesite=Strict",Request::get()->getHost());
			}
		...
		if(!isset($this->isoCode)) {
			if(isset($_COOKIE['GO_LANGUAGE'])) {
				...
				if(!headers_sent()) {
					setcookie("GO_LANGUAGE", $this->isoCode,time() + (10 * 365 * 24 * 60 * 60),"/; samesite=Strict",Request::get()->getHost());
				}
			}
		}
		return $this->isoCode;
