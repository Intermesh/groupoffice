...
go.customfields.type.EncryptedText = Ext.extend(go.customfields.type.Text, {
	...
	renderDetailView: function(value) {
		return value ? `<div style="cursor:pointer" ext:qtip="${Ext.util.Format.htmlEncode(Ext.util.Format.htmlEncode(value))}" onclick="go.util.copyTextToClipboard('${Ext.util.Format.htmlEncode(value)}');window.GOUI.Notifier.notice((Ext.util.Format.htmlEncode(t('Value copied to clipboard')))">${t("Point to view value")}</div>` : "";
	},
	...
	getColumn: function(field) {
		...
		def.renderer = (v) =>{
			return v ? `<div style="cursor:pointer" ext:qtip="${Ext.util.Format.htmlEncode(v)}" onclick="go.util.copyTextToClipboard('${Ext.util.Format.htmlEncode(v)}');window.GOUI.Notifier.notice((Ext.util.Format.htmlEncode(t('Value copied to clipboard')))">${t("Point to view value")}</div>` : "";
		};
