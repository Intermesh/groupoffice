...
class Query extends DbQuery {
   ...
   * @return $this
   */
	public function setModel(string $cls, array $fetchProperties = [], bool $readOnly = false, Property|null$owner = null, int|null $forUserId = null): Query
	{
		$this->model = $cls;
	...
	{
		$stmt = parent::createStatement();

		if(isset($this->model) && $this->getFetchMode() == null) {
			$stmt->fetchTypedModel($this->model, $this->modelConstructorArgs, $this->forUserId);
		}
