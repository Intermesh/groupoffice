...
/**
 * This class is based in the Zend Framework Protocol\Imap class
 * Added GroupOffice logging and changed Zend errors for native RuntimeException
 */
class Imap {
...
//		stream_context_set_option($context, 'ssl', 'verify_peer_name', false);
	...
	protected function nextLine() {
		...
		if ($line === false) {
			// This also happens when the mailserver has an error in the email
			throw new \RuntimeException('cannot read - connection closed?');
		}
		// if($this->idling) {
		// 	$r = [$this->socket]; $w =[]; $e = []; $timeout = 300;
	...
	protected function assumedNextLine($start) {
		$line = $this->nextLine();
		$correct = (strpos($line, $start) === 0);
		if(!$correct) {
			go()->log('Incorrect IMAP response: '.$line);
		}
		return $correct;
	}
	 ...
	 *                                 the decoded tokens
	 * @return void|bool|array tokens if success, false if error, null if bad request
	 */
		...
		foreach ($tokens as $token) {
			if (is_array($token)) {
				...
				if (!$this->assumedNextLine('+ ')) {
					throw new \RuntimeException('cannot send literal string ' . var_export($token,true));
				}
	...
	public function getNamespace() {
		return $this->requestAndResponse('NAMESPACE');
	}
	...
	public function examineOrSelect($command = 'EXAMINE', $box = 'INBOX', $params = []) {
		$tag = null; // define $tag variable before first use
		array_unshift($params, $this->escapeString($box));
		$this->sendRequest($command, $params, $tag);
	...
	public function fetch($items, $from, $to = null, $uid = false) {
		...
		$tokens = null; // define $tokens variable before first use
		while (!$this->readLine($tokens, $tag)) {
			if($tokens[0] == 'VANISHED') {
				$result[$tokens[2]] = null;
			}
	...
	public function uidStore($flags, $uid, $mode = '+') {
		$flags = $this->escapeList($flags);
		$item = $mode.'FLAGS.SILENT';
		return (bool) $this->requestAndResponse('UID STORE', [$uid, $item, $flags], true);
	}
	...
	public function done() {
		if($this->idling) {
			$this->idling = false;
			if (fwrite($this->socket, "DONE\r\n") === false) { // write without TAG
	 ...
	 * @return mixed the created uid or false
	 * @throws \RuntimeException
	 */
	 ...
	 * @return bool success
	 */
	public function copy($folder, $from, $to = null, $uid = false, $move = false) {
		$set = (int) $from;
		if ($to !== null) {
			$set .= ':' . ($to == INF ? '*' : (int) $to);
		}
		$command = ($uid ? "UID " : "").($move ? "MOVE": "COPY");
	 ...
	 * @return bool success
	 */
	public function unsubscribe($folder) {
