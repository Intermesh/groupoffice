...
Ext.calendar.MonthView = Ext.extend(Ext.calendar.CalendarView, {
            ...
            this.clockTask = Ext.TaskMgr.start({
                run: function() {
                    ...
                    if (t.getDay() == this.prevClockDay) {
                        if (el) {
                            el.update(t.format(GO.settings.time_format));
                        }
                    }
                    else {
    ...
    getTemplateEventData: function(evt) {
        ...
        return Ext.applyIf({
            ...
            Title: (evt[M.IsAllDay.name] ? '': evt[M.StartDate.name].format(GO.settings.time_format)) + (!title || title.length == 0 ? '(No title)': title)
        },
        evt);
    },
